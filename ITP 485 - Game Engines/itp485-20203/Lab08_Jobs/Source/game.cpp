...
bool Game::LoadLevel(const WCHAR* fileName)
{
	...
	for (unsigned int i = 0; i < objs.Size(); i++)
	{
		...
		{
			const rapidjson::Value& Properties = obj["components"][j];
			...
			if (type == "Character")
			{
				Character* MyChar = new Character( (SkinnedObj*) ObjectToRender);
				MyChar->LoadProperties(Properties);
				ObjectToRender->AddComponent(MyChar);
			}
		}
