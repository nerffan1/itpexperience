...
void Graphics::InitD3D(HWND hWnd, float width, float height)
{
	...
	//RasterizerState
	mRastaDesc.CullMode = D3D11_CULL_MODE::D3D11_CULL_BACK;
	mRastaDesc.FillMode = D3D11_FILL_MODE::D3D11_FILL_SOLID;
	mRastaDesc.FrontCounterClockwise = true;
	mDev->CreateRasterizerState(&mRastaDesc, &mRastaState);

	// we'll be drwaing triangle lists
	mDevCon->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	//Initiate Z-Buffer
	CreateDepthStencil((int)mScreenWidth, (int)mScreenHeight, &mDS, &mDSView);
	mDSState = CreateDepthStencilState(true, D3D11_COMPARISON_LESS);
	...
	mDev->CreateSamplerState(&sampDesc, &mSState);
	SetActiveSampler(0, mSState);
}

void Graphics::BeginFrame(const Color4& clearColor)
{
	mDevCon->OMSetRenderTargets(1, &mBackBuffer, mDSView);
	mDevCon->ClearRenderTargetView(mBackBuffer, reinterpret_cast<const float*>(&clearColor));
	//Depth Stencil (Z-Buffer)
	mDevCon->OMSetDepthStencilState(mDSState, 1);
	mDevCon->ClearDepthStencilView(mDSView, D3D11_CLEAR_DEPTH, 1.0f, 1);
	mDevCon->OMSetRenderTargets(
		1,
		&mBackBuffer, //???
		mDSView);
	mDevCon->RSSetState(mRastaState);
}
