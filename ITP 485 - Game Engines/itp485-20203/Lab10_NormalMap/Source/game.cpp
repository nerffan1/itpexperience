...
#include "../Source/Components/followCam.h"
#include "Character.h"
#include "JobManager.h"
#include "Profiler.h"
#include "../Source/Components/player.h"
...
void Game::Init(HWND hWnd, float width, float height)
{
	...
	JobManager::GetMan()->Begin();

	//Skinned
	const D3D11_INPUT_ELEMENT_DESC SkinnedInput[] =
	{
	{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(Skinned, pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(Skinned, normal) , D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "BONE", 0, DXGI_FORMAT_R8G8B8A8_UINT, 0, offsetof(Skinned, bones) , D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "WEIGHT", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, offsetof(Skinned, weights) , D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, offsetof(Skinned, uvpos) , D3D11_INPUT_PER_VERTEX_DATA, 0 },
	};

	mManager->SetShader(L"Skinned", new Shader(&mGraphics));
	mManager->GetShader(L"Skinned")->Load(L"Shaders/Skinned.hlsl", SkinnedInput, ARRAY_SIZE(SkinnedInput));
	...
	//Normal Map
	const D3D11_INPUT_ELEMENT_DESC NormalInput[] =
	{
	{ "Position", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(Normal, pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(Normal, normal) , D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TANGENT", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(Normal, tangent) , D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, offsetof(Normal, uvpos) , D3D11_INPUT_PER_VERTEX_DATA, 0 }
	};

	mManager->SetShader(L"Normal", new Shader(&mGraphics));
	mManager->GetShader(L"Normal")->Load(L"Shaders/Normal.hlsl", NormalInput, ARRAY_SIZE(NormalInput));
...
bool Game::LoadLevel(const WCHAR* fileName)
{
	...
	for (unsigned int i = 0; i < objs.Size(); i++)
	{
		...
		Mesh* newMesh = mManager->LoadMesh(MeshFile);
		RenderObj* ObjectToRender;
		Player* myPlayer;
		if (newMesh->IsSkinned())
		{
			ObjectToRender = new SkinnedObj(&mGraphics, newMesh);
		}
		else
		{
			ObjectToRender = new RenderObj(&mGraphics, newMesh);
		}
		//Apply Transforms
		...
		for (rapidjson::SizeType j = 0; j < ComponentArray.Size(); j++)
		{
			const rapidjson::Value& property = obj["components"][j];
			...
			if (type == "PointLight")
			{
				...
				AddLight->LoadProperties(property);
				ObjectToRender->AddComponent(AddLight);
			}
			if (type == "Player")
			{
				myPlayer = new Player(this , dynamic_cast<SkinnedObj*>(ObjectToRender));
				myPlayer->LoadProperties(property);
				ObjectToRender->AddComponent(myPlayer);
			}
			if (type == "FollowCam")
			{
				FollowCam* newCam = new FollowCam(this, ObjectToRender);
				newCam->LoadProperties(property);
				ObjectToRender->AddComponent(newCam);
			}
			if (type == "Collision")
			{
				CollisionComponent* colCom = new CollisionComponent(ObjectToRender);
				ObjectToRender->AddComponent(colCom);
				colCom->LoadProperties(property);
				Physics::AddObj(colCom);
			}
			if (type == "SimpleRotate")
			{
				SimpleRotate* rot = new SimpleRotate(ObjectToRender);
				ObjectToRender->AddComponent(rot);
				rot->LoadProperties(property);
			}
		}
