...
{
	//Functions
	void AddOp(const std::string& s)
	{
		ops.push_back(s);
	}
	std::string GetStackIndex(const std::string& s)
	{
		return std::to_string(stackIndex[s]);
	}
	void PushStack(std::string& s)
	{
		int siz = stackIndex.size();
		stackIndex[s] = siz;
	}
	std::string NextVirtlReg()
	{
		...
		++lastVirtualIndex;
		return ('%' + std::to_string(lastVirtualIndex - 1));

	}
	std::string LastVirReg() { return ('%' + std::to_string(lastVirtualIndex - 1)); }
	// TODO: Add members as needed?
	std::vector<std::vector<std::string>> separatedOps;
	std::vector<std::string> ops;
	std::map<std::string, int> stackIndex;
	int lastVirtualIndex = 0;
};
