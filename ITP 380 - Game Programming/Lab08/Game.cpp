...
CollisionComponent* Game::GetEnemyColl()
{
	return mEnemyColl;
}
...
void Game::UpdateGame()
{
	...
	if (mIntroBuffer > 0)
	{
		...
		if (mIntroBuffer <= 0)
		{
			...
			engine = Mix_PlayChannel(-1, GetSound("Assets/Sounds/Music.ogg"), -1);
		}
	}
}
...
void Game::LoadData()
{
	...
	Mesh* mapMesh = mRenderer->GetMesh("Assets/Track.gpmesh");
	MeshComponent* newMeshComp = new MeshComponent(map);
	newMeshComp->SetMesh(mapMesh);
	map->SetMesh(newMeshComp);
	...
	//Initilalize Players
	mRrival = new Enemy(this);
	mPlayer = new Player(this);

	//Collision
	mEnemyColl = new CollisionComponent(mRrival);
	mEnemyColl->SetSize(35, 35, 35);
	...
	//Intro
	for (Actor* things : mActors)
	{
		things->SetState(ActorState::Paused);
	}
