...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		if (type == "Block")
		{
			...
			bool SetMirror;
			bool SetRot;
			SetMirror = GetBoolFromJSON(actorValue, "mirror", SetMirror);
			SetRot = GetBoolFromJSON(actorValue, "rotates", SetMirror);
			if (SetMirror)
			{
		...
		else if (type == "Player")
		{
			...
			Player* FPlayer = new Player(game, parent);
			game->SetPlayer(FPlayer);
			actor = FPlayer;
			...
			{
				FPlayer->SetRespawn(pos);
			}
		}
		else if (type == "Checkpoint")
		{
			Checkpoint* newCP = new Checkpoint(game, parent);
			actor = newCP;
			std::string levelName;
			std::string TextScreen;
			GetStringFromJSON(actorValue, "level", levelName);
			GetStringFromJSON(actorValue, "text", TextScreen);
			newCP->SetLevelString(levelName);
			newCP->SetTextString(TextScreen);
		}
		else if (type == "Coin")
		{
			Coin* NewCoin = new Coin(game);
			actor = NewCoin;
		}
		else if (type == "SecurityCamera")
		{
			SecurityCamera* NewCam = new SecurityCamera(game);
			actor = NewCam;
			...
			{
				NewCam->SetStartQ(StartQ);
			}
			if (GetQuaternionFromJSON(actorValue, "endQ", EndQ))
			{
				NewCam->SetEndQ(EndQ);
			}
			if (GetFloatFromJSON(actorValue, "interpTime", IntTime))
			{
				NewCam->SetIntTime(IntTime);
			}
			if (GetFloatFromJSON(actorValue, "pauseTime", PauseTime))
			{

				NewCam->SetPauseTime(PauseTime);
			}
		}
